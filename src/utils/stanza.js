...
const st = {
  ...
  async parseMessage(stanza, _converse) {
    // console.log('parseMessage -> ', stanza)
  ...
  async parseMUCMessage(stanza, chatbox, _converse) {
    ...
    if (
      sizzle(`message > forwarded[xmlns="${Strophe.NS.FORWARD}"]`, stanza)
        .length
    ) {
      return new StanzaParseError(
    ...
    const marker = st.getChatMarker(stanza)
    const now = new Date().toISOString()
    const isFromSelf = u.isSameBareJID(nick, _converse.connection.jid)
    if (isFromSelf) return
    // console.log('parseMUCMessage -> ', isFromSelf, stanza)
    /**
  ...
  parseMUCPresence(stanza) {
    // console.log('parseMUCPresence -> ', stanza)
    const from = stanza.getAttribute('from')
