...
converse.plugins.add('converse-bosh', {
  enabled() {
    return !_converse.api.settings
      .get('blacklisted_plugins')
      .includes('converse-bosh')
  },
  ...
  initialize() {
    ...
    async function initBOSHSession() {
      ...
      if (!_converse.bosh_session) {
        ...
        _converse.bosh_session.browserStorage = _converse.createStore(
          id,
          'session'
        )
        await new Promise(resolve =>
          _converse.bosh_session.fetch({ success: resolve, error: resolve })
        )
      }
      if (_converse.jid) {
