...
import { _converse, api, converse } from '../core'
...
converse.plugins.add('converse-roster', {
  ...
  initialize() {
    ...
    _converse.Presence = Model.extend({
      ...
      //     priority =
      //       propertyOf(presence.querySelector('priority'))('textContent') || 0,
      //     resource = this.resources.get(name),
      //     settings = {
      //       name: name,
      //       priority: isNaN(parseInt(priority, 10))
      //         ? 0
      //         : parseInt(priority, 10),
      //       show:
      //         propertyOf(presence.querySelector('show'))('textContent') ||
      //         'online',
      //       timestamp: delay
    ...
    _converse.RosterContact = Model.extend({
      ...
      async initialize(attributes) {
         ...
         */
        await api.trigger('rosterContactInitialized', this, {
          Synchronous: true,
        })
    ...
    _converse.RosterContacts = Collection.extend({
      ...
      comparator(contact1, contact2) {
        ...
        if (
          _converse.STATUS_WEIGHTS[status1] ===
          _converse.STATUS_WEIGHTS[status2]
        ) {
          const name1 = contact1.getDisplayName().toLowerCase()
          const name2 = contact2.getDisplayName().toLowerCase()
          return name1 < name2 ? -1 : name1 > name2 ? 1 : 0
        } else {
          return _converse.STATUS_WEIGHTS[status1] <
            _converse.STATUS_WEIGHTS[status2]
            ? -1
            : 1
        }
      ...
      handleOwnPresence(presence) {
        ...
        if (
          ...
          const status_message = propertyOf(presence.querySelector('status'))(
            'textContent'
          )
          if (status_message) {
      ...
      presenceHandler(presence) {
        ...
        if (this.isSelf(bare_jid)) {
          ...
          sizzle(`query[xmlns="${Strophe.NS.MUC}"]`, presence).length
        ) {
          return // Ignore MUC
        }
    ...
    function updateUnreadCounter(chatbox) {
      const contact =
        _converse.roster &&
        _converse.roster.findWhere({ jid: chatbox.get('jid') })
