...
export default {
  ...
  rooms: {
    ...
    create(jids, attrs = {}) {
      ...
      if (jids === undefined) {
        throw new TypeError(
          'rooms.create: You need to provide at least one JID'
        )
    ...
    async open(jids, attrs = {}, force = false) {
      ...
      if (jids === undefined) {
        ...
        const rooms = await Promise.all(
          jids.map(jid => api.rooms.get(jid, attrs, true))
        )
    ...
    async get(jids, attrs = {}, create = false) {
      async function _get(jid) {
        ...
        // log.error('muc api get', model, create)
        if (!model && create) {
          model = await api.chatboxes.create(jid, attrs, _converse.ChatRoom)
        } else {
          // model =
          //   model && model.get('type') === _converse.CHATROOMS_TYPE
          //     ? model
          //     : null
