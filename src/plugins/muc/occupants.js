...
const ChatRoomOccupants = Collection.extend({
  ...
  async fetchMembers() {
    ...
    const aff_lists = await Promise.all(
      affiliations.map(a => this.chatroom.getAffiliationList(a))
    )
    ...
    const new_nicks = new_members
      .map(m => (!m.jid && m.nick) || undefined)
  ...
  findOccupant(data) {
    ...
    return (
      (jid && this.findWhere({ jid })) || this.findWhere({ nick: data.nick })
    )
  },
})
