  ...
  initialize() {
    ...
    _converse.ChatBoxes = Collection.extend({
      ...
      onConnected(reconnecting) {
        ...
        // log.info('onConnected', _converse.bare_jid)
        this.browserStorage = _converse.createStore(
    ...
    api.listen.on('presencesInitialized', reconnecting =>
      _converse.chatboxes.onConnected(reconnecting)
    )
    api.listen.on('reconnected', () =>
      _converse.chatboxes.forEach(m => m.onReconnection())
    )
    /************************ END Event Handlers ************************/
      ...
      chatboxes: {
        ...
        async create(jids = [], attrs = {}, model) {
          ...
          if (typeof jids === 'string') {
            ...
            return Promise.all(
              jids.map(jid => createChatBox(jid, attrs, model))
            )
          }
        },
        ...
        async get(jids) {
          ...
          if (jids === undefined) {
            ...
            return _converse.chatboxes.models.filter(m =>
              jids.includes(m.get('jid'))
            )
          }
        },
      },
