...
import { Connection, MockConnection } from './shared/connection.js'
...
// These are just the converse-headless-xmpp plugins, for the full converse,
...
const DEFAULT_SETTINGS = {
  ...
  auto_login: true, // Currently only used in connection with anonymous login
  auto_reconnect: true,
  blacklisted_plugins: [],
  ...
  i18n: 'en',
  idle_presence_timeout: 0, // 300, // Seconds after which an idle presence is sent
  jid: undefined,
  keepalive: true,
  loglevel: 'debug',
  locales: ['en', 'zh_CN'],
...
export const i18n = {
   ...
   * support.
   *
   * The default implementation in converse-headless-xmpp simply calls sprintf
...
export const _converse = {
  ...
  default_connection_options: { explicitResourceBinding: false },
  router: new Router(),
...
export const api = (_converse.api = {
  ...
  async trigger(name) {
    ...
    if (options && options.synchronous) {
      ...
      await Promise.all(
        events.map(e => e.callback.apply(e.ctx, args.splice(1)))
      )
    } else {
  ...
  sendIQ(stanza, timeout = _converse.STANZA_TIMEOUT, reject = true) {
    ...
    // console.log('sendIQ -> ', stanza, timeout, reject)
      ...
      if (reject) {
        ...
        promise.catch(e => {
          if (e === null) {
            ...
            console.error('core::sendIQ error : ', err)
          }
        })
      } else {
        promise = new Promise((resolve, reject) =>
...
Object.assign(converse, {
  ...
  async initialize(settings) {
    ...
    return Promise.resolve(_converse)
  },
